...
// forge script script/deploy/mainnet/Mainnet_DeployEOMiddlewareContracts.s.sol --rpc-url $MAINNET_RPC_URL --private-key $PRIVATE_KEY //--broadcast -vvvv
contract Mainnet_DeployEOMiddlewareContracts is Utils, ExistingDeploymentParser {
    string public existingDeploymentInfoPath =
        string(bytes("./script/deploy/mainnet/config/eigenlayer_deployment_mainnet.json"));
    string public deployConfigPath =
        string(bytes("./script/deploy/mainnet/config/middleware_config_mainnet.json"));
    string public outputPath = string.concat(
        "script/deploy/mainnet/output/eoracle_middleware_deployment_data_mainnet.json"
    );
        ...
        )
    {
        ...
        _verifyContractPointers(
            blsApkRegistry, serviceManager, registryCoordinator, indexRegistry, stakeRegistry
        );
        ...
        )
    {
        ...
        strategyAndWeightingMultipliers =
            abi.decode(strategyConfigsRaw, (IEOStakeRegistry.StrategyParams[][]));
    }

    function _parseRegistryCoordinatorParams(string memory config_data)
        internal
        pure
        returns (
            IEORegistryCoordinator.OperatorSetParam[] memory operatorSetParams,
        ...
        )
    {
        ...
        operatorSetParams =
            abi.decode(operatorConfigsRaw, (IEORegistryCoordinator.OperatorSetParam[]));
    ...
    function _verifyContractPointers(
        ...
        require(
            address(_apkRegistry.registryCoordinator()) == address(registryCoordinator),
            "blsApkRegistry.registryCoordinator() != registryCoordinator"
        );

        require(
            address(_indexRegistry.registryCoordinator()) == address(registryCoordinator),
            "indexRegistry.registryCoordinator() != registryCoordinator"
        );

        require(
            address(_stakeRegistry.registryCoordinator()) == address(registryCoordinator),
        ...
        require(
            address(_registryCoordinator.serviceManager()) == address(_serviceManager),
            "registryCoordinator.serviceManager() != _serviceManager"
        );
        require(
            address(_registryCoordinator.stakeRegistry()) == address(stakeRegistry),
            "registryCoordinator.stakeRegistry() != stakeRegistry"
        );
        require(
            address(_registryCoordinator.blsApkRegistry()) == address(_apkRegistry),
            "registryCoordinator.blsApkRegistry() != _apkRegistry"
        );
        require(
            address(_registryCoordinator.indexRegistry()) == address(indexRegistry),
    ...
    function _verifyInitalizations(string memory config_data) internal {
        ...
        require(
            registryCoordinator.ejector() == ejector, "registryCoordinator.ejector() != ejector"
        );
        require(
            registryCoordinator.pauserRegistry() == IPauserRegistry(pauserRegistry),
        ...
        for (uint8 i = 0; i < strategyAndWeightingMultipliers.length; ++i) {
            for (uint8 j = 0; j < strategyAndWeightingMultipliers[i].length; ++j) {
                IEOStakeRegistry.StrategyParams memory strategyParams =
                    stakeRegistry.strategyParamsByIndex(i, j);
                require(
                ...
                require(
                    strategyParams.multiplier == strategyAndWeightingMultipliers[i][j].multiplier,
                    "stakeRegistry.strategyAndWeightingMultipliers != strategyAndWeightingMultipliers"
                );
            }
        }
